...
export function SidebarCategories({ collapsed, activeTab, onTabChange, activeCategory, onSelectCategory }: SidebarCategoriesProps) {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const record = { id, userId: user.uid, name: newName.trim(), createdAt: timestamp, updatedAt: timestamp };
    ...
    return (
        <div className="relative group px-0">
            <div className="flex items-center w-full relative">
                <button
                    onClick={onSelect}
                    className={`w-full flex items-center rounded-xl text-[13px] font-medium transition-all gap-3.5 px-3 py-2.5 ${isActive ? 'bg-gray-900 text-white shadow-md' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-700'}`}
